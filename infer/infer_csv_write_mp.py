...
def write_csv_process(savepath, data_queue):

    print(f"write csv process pid: {os.getpid()}")
    ...
    with open(savepath, "w") as fd:
        ...
        while True:
            ...
            writer.writerow([f, r, p])

    print("write csv process finished")
...
def main(config, outputdir):
    ...
    data_queue = Queue()
    write_process = Process(target=write_csv_process, args=(p, data_queue))
    write_process.start()
        ...
        for data in tqdm(infer_dataloader):
            ...
            with torch.no_grad():
                ...
                for f, r, p in zip(
                    ...
                    fn = get_fn(f)
                    data_queue.put((fn, r.item(), p.item()))
